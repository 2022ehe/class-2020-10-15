...
ggplot(x, aes(x = paddle_red, y = urn_red)) + 
  geom_point(alpha = 0.01) +
  labs(title = 'Joint Distribution of Red Beads in Paddle and Urn',
       x = 'Number of Red Beads in Paddle',
       y = 'Number of Red Beans in Urn') + 
  theme_classic()

# For each number of beads in the paddle, we get a range of possible number of 
# red beads in the urn.
```
...
ggplot(post, aes(x = urn_red, y = probs)) + 
  geom_col() + 
  labs(title = 'Posterior Probability Density for Number of Red Beans in Urn
       Given 10 Red Beads were Sampled in a Paddle of Size 25',
       x = 'Number of Red Beads in Urn',
       y = 'Density')
```
...
```{r sc3}
a <- tibble(urn_red = seq(0, 1000, by = 10)) %>% 
...
tibble(total = nrow(a), greater_3 = nrow(b), chance = greater_3/total)
  #mutate(greater_3 = paddle_red > 3) %>%
  #mutate(prob = sum(greater_3 == TRUE) / nrow(urn_red))
  #summarize(prob = sum(greater_3 == TRUE)/nrow(urn_red))

```
```{r sc3}
post %>%
  mutate(new_reds = map(urn_red, ~ rbinom(n = 100, size = 25, prob = ./1000))) %>%
  unnest(new_reds) %>%
  mutate(res = ifelse(new_reds > 3, 1, 0)) %>%
  mutate(weighted_res = probs * res) %>%
  summarize(final = sum(weighted_res)/sum(probs))
```
